...
def register(request):
    if request.method == "POST":
        ...
        if not username or get_user_model().objects.filter(username=username).exists():
            messages.error(
                request, 'Vartotojo vardas neįvestas, arba toks vartotojas jau egzistuoja.')
            is_error = True
        if not email or get_user_model().objects.filter(email=email).exists():
            messages.error(
                request, 'El.pašto adresas neįvestas, arba vartotojas su įvestu el.pašto adresu jau egzistuoja.')
            is_error = True
        ...
        try:
            ...
            messages.error(
                request, 'Slaptažodio klaida: {}'.format(password_error[0]))
            is_error = True
        if is_error:
            ...
            messages.success(
                request, 'Vartotojas {} sukurtas sėkmingai'.format(username))
